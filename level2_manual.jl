...
function generate_data(len)
    # Создаем массив нужного размера с явным типом Float64
...
@time generate_data(1_000_000)
println("\nЗапуск профилировщика для оптимизированной версии...")
ProfileView.@profview generate_data(100_000)
...
y[i] = x[i] - x[i-1]
=#
function first_diff(x::Vector)# Аргумент: x::Vector - массив чисел (любого типа)
    n = length(x)
    y = zeros(n)  # Создаем массив y с нулями такой же длины как n
    for i in 2:n # Цикл со 2-го элемента до последнего (индексация с 1)
        y[i] = x[i] - x[i-1]
    end
    return y # y[1] остается 0 (первый элемент не имеет предыдущего)
end
...
=#
arr = [1, 2, 3, 4, 5]
push!(arr, 1000)        # в конец
pushfirst!(arr, 9999)   # в начало
insert!(arr, 4, 555) # в середину
println(arr)
...
=#
module Foo
    # Экспортируем  функцию bar
    export bar
    # Внутренняя функция 
    function bar(x)
        return x * 2
    end
    # Приватная функция (доступна только внутри модуля)
    function baz(x)
        return x ^ 2
    end
end
# using .Foo ?
using .Foo          # импортируем модуль
bar(5)              # работает (экспортирована)
# baz(5)            # ошибка (не экспортирована)
Foo.baz(5)          # работает (полное имя)
# import .Foo ?
import .Foo
Foo.bar(5)          
Foo.baz(5)        
...
# пример
Pkg.activate("data_analysis") # Переключились
Pkg.add("DataFrames")         # Установили DataFrames
Pkg.add("CSV")                # Установили CSV

using DataFrames              
Pkg.activate()           # Вернулись к глобальному
...
# save("имя_файла", "имя_переменной_в_файле", переменная)
# Загружаем из файла
loaded = load("data.jld2", "data")
# load("файл", "имя_переменной_которую_достать")
#пример 2 
user_name = "Alice"
user_age = 25
user_scores = [100, 95, 88]

# Сохраняем в ОДИН файл с РАЗНЫМИ именами
save("user.jld2", 
     "name", user_name,      # сохраняем под именем "name"
     "age",  user_age,       # сохраняем под именем "age"  
     "scores", user_scores)  # сохраняем под именем "scores"

# Загружаем обратно
loaded_name = load("user.jld2", "name")    # "Alice"
loaded_age = load("user.jld2", "age")      # 25
loaded_scores = load("user.jld2", "scores") # [100,95,88]
# Сохранить и загрузить табличные объекты (массивы) в CSV, сравнить их
#CSV = формат файла (как .txt, .docx)
#DataFrame = структура данных в памяти (как массив, список)
...
=#
# Активировать окружение из папки
#julia --project=.

# Активировать конкретное окружение
...
#julia -e 'x = 10; y = 20; println(x + y)'

# С подключением пакета
